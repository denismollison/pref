...
#'
#sinstall()c22=stv.batch("Scottish Council election 2022","SC2022","SC2022out")

stv.batch=function(election,indirec,outdirec=tempdir(),plot=TRUE,verbose=FALSE){
...
#  wards=readLines(paste(inarea,"wards",sep="/"))
 wards2=system(paste("ls",inarea),intern=T)
 nw=length(wards2);  wards=character()
 
for(iw in 1:nw){
     wards2[[iw]]=strsplit(wards2[[iw]],".blt")[[1]]
...
for(iw in 1:nw){
ward2=wards2[[iw]]
datafile=paste0(inarea,"/",ward2,".blt")
...
# add link to map - details specific to SC2022- rethink for general prog !
map=paste0("https://boundaries.scot/Electoral/5th_Reviews/",areas3[[ia]],"/5th_Review_",areas3[[ia]],"_Ward_",wn,".pdf")
