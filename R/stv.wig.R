...
total_vote=sum(mult)
q0=ceiling((total_vote+1)/(ns+1)); qa=q0; qpc=100*q0/total_vote
...
for(iv in 1:nv){
    ...
    vm[,nc+1]=c(ff,0)-apply(vm[,1:nc],1,sum)   # assumes no n-t fps
    v=apply(vm,2,sum)
#   print(round(vm,1))
...
if(plot==T){
  ...
  h=600+200*transf
#   cat("stv.wig line 191 - vm\n")
 #  round(print(vm),2)
#   cat("stv.wig line 191 - vmp\n")
 #  round(print(vmp),2)
